...
| **M2** ◑ | V_j transforms (all 5) + **F₂-linearity bridge lemma** + multiplicativity (all ~10 (j,P) instances). **(A) DONE**: all 10 basis-chain instances `V_j(baseP⋆δ_p)=P̂_j·V_j(δ_p)` native_decide GREEN (`MultProbe.lean`) — conventions certified, no flip. **(B) remains**: the F₂-linearity bridge `V_j(f+g)=V_j f+V_j g` to lift basis→∀z (the ~100-line delicate piece; design tension: `Fin 4`+tables keep native_decide cheap but block a mathlib `AddCommMonoid` route, so additivity is a hand-rolled char-2 foldl induction) | hybrid | med | (A) ✅ 144-case native_decide ×10 GREEN; (B) `∀z` bridge pending |
