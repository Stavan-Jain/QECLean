...
non-degenerate in that sense.

**Every Bravyi-table instance is degenerate** (verified with this
module's `support_subgroup_index`): for gross, `supp(A) = {(3,0),
(0,1), (0,2)}` generates the order-24 subgroup `⟨(3,0)⟩ × Z_6` inside
`Z_12 × Z_6`, so `[G : ⟨supp(A)⟩] = 3`, and symmetrically for `B`.
The same index 3 holds across the whole Bravyi table — degeneracy is
*engineered in*, not avoided; the factor 3 is exactly Lin–Pryadko's
`c = |G_a ∩ G_b|` and is part of what gives the lifted-product
wraparound its distance. See HANDOFF.md §6i. (An earlier version of
this docstring claimed the gross polynomials cover the full group;
that was false.)
