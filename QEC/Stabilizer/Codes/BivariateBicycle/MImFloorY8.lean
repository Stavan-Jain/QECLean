/-
# Phase 6: the safe-sector floor for y-orbit representative 8

Instantiates the general `floor_of_data` (`MImMembership §14c`) for y-orbit rep 8, whose
`ker ∂₂` element is `kcombo 1 1 1 1 0 0`.  Supplies the seam offsets (`oL = rightHalf`,
`oR = leftHalf`), the offData / bound `native_decide`s, and the heavy floor leaf
`floorOK oL oR = true`.  Generated by `experiments/bb_lab/scripts/gen_yrep_module.py 8`.
-/
...
namespace Quantum.Stabilizer.Homological.BB.LightStab.Y8

-- `decide` (kernel) needs more recursion headroom than compiled `native_decide`.
set_option maxRecDepth 40000

/-- Y-orbit-8 representative `ker ∂₂` element. -/
def zrep : BaseGroup → ZMod 2 := kcombo 1 1 1 1 0 0

/-- Side-0 (rightHalf) seam offsets, flat `j*4+s`. -/
def oL : Array Nat := #[0,0,0,0,3,2,1,3,0,0,0,0,3,3,1,1,2,1,1,3]
/-- Side-1 (leftHalf) seam offsets, flat `j*4+s`. -/
def oR : Array Nat := #[0,0,0,0,0,1,3,2,0,0,0,0,3,2,2,3,1,2,1,1]
...
/-- **Y-orbit-8 safe-sector floor**: every base 1-cycle in `[seamC zrep]` has weight `≥ 12`. -/
...
end Quantum.Stabilizer.Homological.BB.LightStab.Y8
