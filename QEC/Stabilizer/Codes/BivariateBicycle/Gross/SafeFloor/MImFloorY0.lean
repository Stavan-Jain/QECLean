...
Generator : qec-lab:experiments/bb_lab/scripts/gen_yrep_module.py (arg: 0)
Regen     : from a sibling qec-lab checkout (QECLEAN_ROOT points here):
            cd qec-lab:experiments/bb_lab && uv run python scripts/gen_yrep_module.py 0
To change this file, change the generator in qec-lab and regenerate — land both repos' changes together.
-/
/-
# Phase 6: the safe-sector floor for y-orbit representative 0

Instantiates the general `floor_of_data` (`MImMembership §14c`) for y-orbit rep 0, whose
`ker ∂₂` element is `kcombo 1 0 0 0 0 0`.  Supplies the seam offsets (`oL = rightHalf`,
`oR = leftHalf`), the offData / bound `native_decide`s, and the heavy floor leaf
`floorOK oL oR = true`.  Generated by `qec-lab:experiments/bb_lab/scripts/gen_yrep_module.py 0`.
-/
...
def oL : Array Nat := #[0,0,0,0,3,0,3,0,0,0,0,0,3,0,3,0,2,0,2,0]
...
/-- **Y-orbit-0 safe-sector floor**: every base 1-cycle in `[seamC zrep]` has weight `≥ 12`. -/
