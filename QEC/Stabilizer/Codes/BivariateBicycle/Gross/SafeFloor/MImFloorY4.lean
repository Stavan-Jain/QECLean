...
Generator : qec-lab:experiments/bb_lab/scripts/gen_yrep_module.py (arg: 4)
Regen     : from a sibling qec-lab checkout (QECLEAN_ROOT points here):
            cd qec-lab:experiments/bb_lab && uv run python scripts/gen_yrep_module.py 4
To change this file, change the generator in qec-lab and regenerate — land both repos' changes together.
-/
/-
# Phase 6: the safe-sector floor for y-orbit representative 4

Instantiates the general `floor_of_data` (`MImMembership §14c`) for y-orbit rep 4, whose
`ker ∂₂` element is `kcombo 1 1 1 0 0 0`.  Supplies the seam offsets (`oL = rightHalf`,
`oR = leftHalf`), the offData / bound `native_decide`s, and the heavy floor leaf
`floorOK oL oR = true`.  Generated by `qec-lab:experiments/bb_lab/scripts/gen_yrep_module.py 4`.
-/
...
def oL : Array Nat := #[0,0,0,0,1,2,2,0,0,0,0,0,1,3,2,0,1,1,3,0]
/-- Side-1 (leftHalf) seam offsets, flat `j*4+s`. -/
def oR : Array Nat := #[0,0,0,0,1,3,0,2,0,0,0,0,2,1,0,3,0,3,0,1]
...
/-- **Y-orbit-4 safe-sector floor**: every base 1-cycle in `[seamC zrep]` has weight `≥ 12`. -/
