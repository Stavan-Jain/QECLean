...
Generator : experiments/bb_lab/scripts/gen_yrep_module.py (arg: 1)
Regen     : cd experiments/bb_lab && uv run python scripts/gen_yrep_module.py 1
To change this file, change the generator and regenerate — in the same PR.
-/
/-
# Phase 6: the safe-sector floor for y-orbit representative 1

Instantiates the general `floor_of_data` (`MImMembership §14c`) for y-orbit rep 1, whose
`ker ∂₂` element is `kcombo 0 0 1 0 0 0`.  Supplies the seam offsets (`oL = rightHalf`,
`oR = leftHalf`), the offData / bound `native_decide`s, and the heavy floor leaf
`floorOK oL oR = true`.  Generated by `experiments/bb_lab/scripts/gen_yrep_module.py 1`.
-/
...
def oL : Array Nat := #[0,0,0,0,0,2,3,0,0,0,0,0,0,3,3,0,0,1,2,0]
/-- Side-1 (leftHalf) seam offsets, flat `j*4+s`. -/
def oR : Array Nat := #[0,0,0,0,0,2,1,3,0,0,0,0,3,3,1,1,1,1,1,3]
...
/-- **Y-orbit-1 safe-sector floor**: every base 1-cycle in `[seamC zrep]` has weight `≥ 12`. -/
