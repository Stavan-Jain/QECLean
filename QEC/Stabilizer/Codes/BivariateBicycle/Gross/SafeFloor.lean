...
`MImMembership`, the `2³⁰` `floorOK` walk) has been retired
...
- `MImTransport`  — the translation symmetry: `seamC` y-covariance, the x cut-shift defect,
                    ...
                    `(j,k)`-translate
- `LightFloor`    — **(Tier 3, A4 §§12–13)** the coupled light-orbit certificate: the packed
                    ring frame, the spine-cell checker (`killOK`) carrying Prop 30
                    (`min_L + min_R ≥ 10` on all `1024` cells) and Prop 31 (the ρ-link kill of
                    the `10`-tight cells), the coset extraction (`coset_ex`) and the soundness
                    bridge (`floor_of_killOK`)
- `MImFloorY{0,1,4,11,12}` — the safe-sector floor proven for the 5 full-translation-orbit
                    representatives, **all analytic**: the **light orbits** `Y0, Y1, Y4`
                    (weights 16/18a/18b) by the coupled spine certificate
                    (`LightFloor.floor_of_killOK`), the **weight-24 orbits** `Y11, Y12` by the
                    decoupled slot-frame walk
                    (`WtFloor24Bridge.costFromComps_ge_12_of_blocks`).  No `floorOK` leaf
