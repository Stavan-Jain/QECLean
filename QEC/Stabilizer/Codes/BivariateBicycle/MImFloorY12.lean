/-
# Phase 6: the safe-sector floor for y-orbit representative 12

Instantiates the general `floor_of_data` (`MImMembership §14c`) for y-orbit rep 12, whose
`ker ∂₂` element is `kcombo 1 1 0 1 1 0`.  Supplies the seam offsets (`oL = rightHalf`,
`oR = leftHalf`), the offData / bound `native_decide`s, and the heavy floor leaf
`floorOK oL oR = true`.  Generated by `experiments/bb_lab/scripts/gen_yrep_module.py 12`.
-/
...
/-- **Y-orbit-12 safe-sector floor**: every base 1-cycle in `[seamC zrep]` has weight `≥ 12`. -/
